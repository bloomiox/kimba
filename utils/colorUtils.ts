...
export const mapToAccentColor = (baseClass: string): string => {
  ...
  const matches = baseClass.match(
    /(^|\s)(((hover|focus|dark|group-hover|dark:group-hover):)*(bg|text|border|ring))-[a-z]+-\d+/g
  );
  ...
  matches.forEach(match => {
    ...
    const parts = match.match(
      /(^|\s)((((hover|focus|dark|group-hover|dark:group-hover):)*)(bg|text|border|ring))-[a-z]+-\d+/
    );
