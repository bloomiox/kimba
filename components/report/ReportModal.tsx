...
const ReportModal: React.FC<ReportModalProps> = ({ reportType, data, dateRange, onClose }) => {
    ...
    const renderReport = () => {
        switch (reportType) {
            case 'salesSummary': return <SalesSummaryReport sales={data.sales} />;
            case 'servicePerformance': return <ServicePerformanceReport appointments={data.appointments} services={data.services} />;
            case 'dailyCloseout': return <DailyCloseoutReport sales={data.sales} />;
            case 'clientList': return <ClientListReport clients={data.clients} />;
            default: return null;
        }
    };
      ...
      <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 print:hidden">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-4xl h-[90vh] p-6 relative flex flex-col animate-fade-in">
          <div className="flex-shrink-0 flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{reportTitles[reportType]}</h2>
            <div className="flex gap-2">
                <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-accent text-white rounded-lg font-semibold"><PrinterIcon className="w-5 h-5"/> Print / Save as PDF</button>
                <button onClick={onClose} className="text-gray-500 hover:text-gray-800 dark:hover:text-white"><CloseIcon className="w-8 h-8" /></button>
            </div>
          </div>
          <div className="flex-grow overflow-y-auto bg-gray-50 dark:bg-gray-900/50 p-6 rounded-lg printable-area">
            ...
            <div className="report-content">
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-300">{reportTitles[reportType]}</h2>
