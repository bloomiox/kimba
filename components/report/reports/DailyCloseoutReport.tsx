...
const DailyCloseoutReport: React.FC<DailyCloseoutReportProps> = ({ sales }) => {
  ...
  const reportData = useMemo(() => {
    ...
    for (const sale of sales) {
      ...
      salesByHairstylist[sale.hairstylistId] =
        (salesByHairstylist[sale.hairstylistId] || 0) + sale.total;
    }
    ...
    <div className="max-w-xl mx-auto">
      <div className="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
        <div className="flex justify-between text-xl font-bold py-2 border-b-2 border-black dark:border-gray-400">
          ...
          <span>
            {reportData.totalSales.toLocaleString(langCode, { style: 'currency', currency })}
          </span>
        </div>
        <div className="flex justify-between text-sm py-1">
          <span>Total Tips</span>
          <span>
            {reportData.totalTips.toLocaleString(langCode, { style: 'currency', currency })}
          </span>
        </div>
        <div className="flex justify-between text-sm py-1">
          <span>Total VAT</span>
          <span>
            {reportData.totalVat.toLocaleString(langCode, { style: 'currency', currency })}
          </span>
        </div>
      </div>
