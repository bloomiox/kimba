...
const ServicePerformanceReport: React.FC<ServicePerformanceReportProps> = ({ appointments, services }) => {
    const { t } = useSettings();
            ...
            <table className="w-full text-left text-sm">
                ...
                <tbody>
                    {reportData.map(service => (
                        <tr key={service.id} className="border-b dark:border-gray-700/50 even:bg-gray-100 dark:even:bg-gray-900/50">
                            <td className="p-2 font-medium">{service.name}</td>
                            <td className="p-2 text-right">{service.count}</td>
                            <td className="p-2 text-right font-semibold">{service.revenue.toLocaleString(langCode, {style: 'currency', currency})}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
