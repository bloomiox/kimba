...
const SalesSummaryReport: React.FC<SalesSummaryReportProps> = ({ sales }) => {
    ...
    const reportData = useMemo(() => {
        ...
        for (const sale of sales) {
            ...
            if (!salesByPayment[sale.paymentMethod]) salesByPayment[sale.paymentMethod] = { count: 0, total: 0 };
            salesByPayment[sale.paymentMethod].count++;
            salesByPayment[sale.paymentMethod].total += sale.total;
            
            if (!salesByHairstylist[sale.hairstylistId]) salesByHairstylist[sale.hairstylistId] = { count: 0, total: 0 };
            salesByHairstylist[sale.hairstylistId].count++;
            salesByHairstylist[sale.hairstylistId].total += sale.total;
        }

        return { grossSales, totalDiscounts, netSales, totalVat, totalTips, totalCollected, salesByPayment, salesByHairstylist };
    }, [sales]);
    ...
    const SummaryRow: React.FC<{label: string, value: number}> = ({label, value}) => (
        <div className="flex justify-between py-2 border-b dark:border-gray-700/50">
            ...
            <span className="font-bold">{value.toLocaleString(langCode, {style: 'currency', currency})}</span>
        </div>
    );
    
    const renderBreakdownTable = (title: string, data: Record<string, {count: number; total: number}>, nameMap?: Record<string, string>) => (
        <div>
            ...
            <table className="w-full text-left text-sm">
                ...
                <tbody>
                    {Object.entries(data).map(([key, value]) => (
                         <tr key={key} className="border-b dark:border-gray-700/50 even:bg-gray-100 dark:even:bg-gray-900/50">
                            <td className="p-2 capitalize">{nameMap ? nameMap[key] : key}</td>
                            <td className="p-2 text-right">{value.count}</td>
                            <td className="p-2 text-right font-semibold">{value.total.toLocaleString(langCode, {style: 'currency', currency})}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
        ...
        <div className="space-y-4">
            <div className="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
                ...
                <div className="flex justify-between py-2 font-bold text-xl mt-2">
                    ...
                    <span>{reportData.totalCollected.toLocaleString(langCode, {style: 'currency', currency})}</span>
                </div>
            </div>
            
            {renderBreakdownTable('Sales by Payment Method', reportData.salesByPayment)}
            {renderBreakdownTable('Sales by Hairstylist', reportData.salesByHairstylist, hairstylistsById)}
        </div>
    );
};
