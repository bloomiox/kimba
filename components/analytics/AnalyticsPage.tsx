...
const MetricsView: React.FC = () => {
          ...
          {data.map((value, index) => (
            <div key={index} className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className="w-full h-full bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden flex items-end">
                <div
                  className="w-full bg-accent animate-bar-rise"
                  style={{
  ...
  const dayLabels = [
    ...
    new Date(2024, 0, 5),
    new Date(2024, 0, 6),
  ].map(d => d.toLocaleString(langCode, { weekday: 'short' }));
  const timeBlockLabels = [
    t('analytics.timeBlocks.morning'),
    t('analytics.timeBlocks.afternoon'),
    t('analytics.timeBlocks.evening'),
  ];
  const timeBlockData = [
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <KPICard
          icon={DollarSignIcon}
          title="Total Revenue"
          value={filteredData.totalRevenue.toLocaleString(langCode, {
            style: 'currency',
            currency: currency,
          })}
        ...
        <KPICard
          ...
          value={filteredData.avgRevenue.toLocaleString(langCode, {
            style: 'currency',
            currency: currency,
          })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <Card title={t('analytics.topServices')}>
            <div className="space-y-3">
              {filteredData.topServices.map(service => (
                <div key={service.name} className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-gray-600 dark:text-gray-300">
                    {service.revenue.toLocaleString(langCode, {
                      style: 'currency',
                      currency: currency,
                    })}
                  </span>
              ...
              {filteredData.topServices.length === 0 && (
                <p className="text-center text-gray-500 py-4">{t('analytics.noData')}</p>
              )}
            </div>
          </Card>
        </div>
        <div className="lg:col-span-2">
          <BarChart
            data={filteredData.bookingsByDay}
            labels={dayLabels}
            title={t('analytics.bookingsByDay')}
          />
        </div>
      </div>
      <div className="grid grid-cols-1">
        <BarChart
          ...
          title={t('analytics.bookingsByTime')}
        />
      </div>
    </div>
  );
...
const SalesView: React.FC = () => {
  ...
  const [sort, setSort] = useState<'date-desc' | 'date-asc' | 'total-desc' | 'total-asc'>(
    'date-desc'
  );
  ...
  const clientsById = useMemo(
    () => clients.reduce((acc, c) => ({ ...acc, [c.id]: c.name }), {} as Record<string, string>),
    [clients]
  );
      ...
      <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 relative animate-fade-in">
          ...
          <div className="space-y-2 text-sm">
            <p>
              <strong>{t('sales.receipt.date')}:</strong>{' '}
              {new Date(sale.createdAt).toLocaleString(langCode, {
                dateStyle: 'medium',
                timeStyle: 'short',
              })}
            </p>
            <p>
              <strong>{t('pos.customer')}:</strong>{' '}
              {sale.clientId ? clientsById[sale.clientId] : t('pos.walkIn')}
            </p>
            <p>
              <strong>{t('pos.hairstylist')}:</strong> {hairstylistsById[sale.hairstylistId]}
            </p>
          </div>
          ...
          <div className="mt-2 pt-2 border-t dark:border-gray-600 text-sm space-y-1">
            <div className="flex justify-between">
              ...
              <span>{sale.subtotal.toLocaleString(langCode, { style: 'currency', currency })}</span>
            </div>
            {sale.discount && sale.discount.amount > 0 && (
              <div className="flex justify-between text-green-600 dark:text-green-400">
                <span>
                  {t('sales.receipt.discount')} ({sale.discount.reason})
                </span>
                <span>
                  - {sale.discount.amount.toLocaleString(langCode, { style: 'currency', currency })}
                </span>
              </div>
            )}
            <div className="flex justify-between">
              <span>
                {t('pos.summary.vat')} ({sale.vatRate}%)
              </span>
              <span>
                {sale.vatAmount.toLocaleString(langCode, { style: 'currency', currency })}
              </span>
            </div>
            <div className="flex justify-between">
              <span>{t('pos.summary.tip')}</span>
              <span>{sale.tip.toLocaleString(langCode, { style: 'currency', currency })}</span>
            </div>
            <div className="flex justify-between font-bold text-base mt-2 pt-2 border-t dark:border-gray-700">
              <span>{t('pos.summary.total')}</span>
              <span>{sale.total.toLocaleString(langCode, { style: 'currency', currency })}</span>
            </div>
          </div>
          <div className="mt-4 text-center text-xs text-gray-500 capitalize">
            {t('sales.receipt.paidVia')} {sale.paymentMethod}
          </div>
        </div>
      </div>
    );
            ...
            <table className="w-full text-left">
              <thead className="border-b dark:border-gray-700">
                <tr>
                  ...
                  <th className="p-3 text-sm font-semibold text-gray-500 dark:text-gray-400 hidden md:table-cell">
                    {t('sales.table.stylist')}
                  </th>
                  <th className="p-3 text-sm font-semibold text-gray-500 dark:text-gray-400 hidden sm:table-cell">
                    {t('sales.table.payment')}
                  </th>
                  <th className="p-3 text-sm font-semibold text-gray-500 dark:text-gray-400 text-right">
                    {t('sales.table.total')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {sortedSales.map(sale => (
                  <tr
                    ...
                    className="border-b dark:border-gray-700/50 hover:bg-gray-100/50 dark:hover:bg-gray-900/40 cursor-pointer"
                  >
                    ...
                    <td className="p-3 text-sm">
                      {sale.clientId ? (
                        clientsById[sale.clientId]
                      ) : (
                        <span className="italic text-gray-500">{t('pos.walkIn')}</span>
                      )}
                    </td>
                    <td className="p-3 text-sm hidden md:table-cell">
                      {hairstylistsById[sale.hairstylistId]}
                    </td>
                    <td className="p-3 text-sm hidden sm:table-cell capitalize">
                      {sale.paymentMethod}
                    </td>
                    <td className="p-3 font-semibold text-right">
                      {sale.total.toLocaleString(langCode, { style: 'currency', currency })}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <Card key={report.id}>
            <div className="flex flex-col h-full">
              ...
              <div className="mt-4 pt-4 border-t dark:border-gray-700 flex-grow flex items-end">
                <Button
                  onClick={() => setViewingReport(report.id as ReportType)}
                  className="w-full"
                >
                  Generate Report
                </Button>
...
const AnalyticsPage: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div
        className={`flex-grow overflow-y-auto animate-fade-in ${activeTab === 'metrics' ? 'block' : 'hidden'}`}
      >
        <MetricsView />
      </div>
      <div
        className={`flex-grow overflow-y-auto animate-fade-in ${activeTab === 'sales' ? 'block' : 'hidden'}`}
      >
        <SalesView />
      </div>
      <div
        className={`flex-grow overflow-y-auto animate-fade-in ${activeTab === 'reports' ? 'block' : 'hidden'}`}
      >
        <ReportView />
      </div>
      <style>{`
