...
const TeamAnalytics: React.FC = () => {
  ...
  const [selectedMetric, setSelectedMetric] = useState<'revenue' | 'appointments' | 'commission'>(
    'revenue'
  );
  ...
  const getMetricLabel = (metric: string) => {
    switch (metric) {
      case 'revenue':
        return t('team.totalRevenue');
      case 'appointments':
        return t('team.totalAppointments');
      case 'commission':
        return t('team.totalCommission');
      default:
        return metric;
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t('team.analytics.activeStylists')}
              </p>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center overflow-hidden">
                      {item.hairstylist.photoUrl ? (
                        <img
                          src={item.hairstylist.photoUrl}
                          alt={item.hairstylist.name}
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-12 h-12 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center overflow-hidden">
                    {performer.hairstylist.photoUrl ? (
                      <img
                        src={performer.hairstylist.photoUrl}
                        alt={performer.hairstylist.name}
                        className="w-full h-full object-cover"
                      />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      #{performer.rank} {getMetricLabel(selectedMetric)}
                    </p>
                  </div>
                </div>
