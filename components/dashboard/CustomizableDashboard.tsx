...
const WidgetLibrary = lazy(() => import('./WidgetLibrary'));
...
const CustomizableDashboard: React.FC<CustomizableDashboardProps> = ({
  ...
  const [layoutAnalytics, setLayoutAnalytics] = useState<GridMetrics>({
    density: 0,
    efficiency: 0,
    alignment: 0,
    balance: 0,
  });
  const [draggedWidgetSize, setDraggedWidgetSize] = useState<{
    width: number;
    height: number;
  } | null>(null);
  ...
  const lastSavedConfigRef = useRef<DashboardConfiguration | null>(null);

  // Load dashboard configuration from settings context only on initial load
  useEffect(() => {
    logger.debug('🔍 Dashboard config useEffect triggered:', {
      savedDashboardConfig: !!savedDashboardConfig,
      hasInitialized,
      savedConfigWidgetCount: savedDashboardConfig?.widgets?.length || 0,
    });

    if (savedDashboardConfig && !hasInitialized) {
      logger.debug('📥 Loading dashboard config from context (initial load)');
      setDashboardConfig(savedDashboardConfig);
      lastSavedConfigRef.current = savedDashboardConfig;
      setHasInitialized(true);
    } else if (savedDashboardConfig && hasInitialized) {
      logger.debug('⚠️ Ignoring dashboard config update (already initialized)');
    }
  }, [savedDashboardConfig, hasInitialized]);

  // Save dashboard configuration to Supabase
  const saveDashboardConfig = useCallback(
    async (newConfig: DashboardConfiguration) => {
      logger.debug('🔄 saveDashboardConfig called with:', newConfig);
      ...
      try {
        ...
        logger.debug('✅ Dashboard configuration saved to Supabase successfully');
      } catch (error) {
        ...
        if (lastSavedConfigRef.current) {
          logger.warn('🔄 Reverting to last known good state');
          setDashboardConfig(lastSavedConfigRef.current);
        }
      }
    },
    [updateSettings]
  ...
  const calculateLayoutBalance = (widgets: DashboardWidget[]): number => {
    ...
    widgets.forEach(widget => {
      ...
      const distance = Math.sqrt(
        Math.pow(widgetCenterX - centerX, 2) + Math.pow(widgetCenterY - centerY, 2)
      );
  ...
  const canPlaceWidget = (
    ...
    if (
      x < 0 ||
      y < 0 ||
      x + width > dashboardConfig.gridCols ||
      y + height > dashboardConfig.gridRows
    ) {
      return false;
    }
      ...
      for (let dx = -1; dx <= size.width; dx++) {
        ...
        if (
          widgets.some(
            w =>
              checkX >= w.position.x &&
              checkX < w.position.x + w.size.width &&
              checkY >= w.position.y &&
              checkY < w.position.y + w.size.height
          )
        ) {
          bonus += 5;
        }
  ...
  const findOptimalPositionInGrid = (
    grid: (string | null)[][],
    size: { width: number; height: number }
  ): { x: number; y: number } => {
  ...
  const getSnapPosition = (
    ...
    if (
      gridX >= 0 &&
      gridX < dashboardConfig.gridCols &&
      gridY >= 0 &&
      gridY < dashboardConfig.gridRows
    ) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (sourceWidgetId && dragOverPosition && draggedWidgetSize) {
      ...
      const otherWidgets = dashboardConfig.widgets.filter(
        w => w.id !== sourceWidgetId && w.isVisible
      );
      ...
      logger.info(
        `✅ Widget "${sourceWidget.title}" moved to position (${newPosition.x}, ${newPosition.y})`
      );
      if (conflictingWidgets.length > 0) {
        logger.debug(`📦 Displaced ${conflictingWidgets.length} conflicting widgets`);
      }
    }
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            {isCustomizing && (
              ...
              >
                <svg
                  className={`w-4 h-4 ${isCompacting ? 'animate-spin' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3">
                      ...
                      <p className="text-blue-700 dark:text-blue-300">
                        Drag widgets to see intelligent snap indicators
                      </p>
                    </div>
                    <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3">
                      <div className="font-semibold text-blue-800 dark:text-blue-200 mb-1 flex items-center gap-2">
                      ...
                      <p className="text-blue-700 dark:text-blue-300">
                        Click to automatically optimize layout
                      </p>
                    </div>
                    <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3">
                      <div className="font-semibold text-blue-800 dark:text-blue-200 mb-1 flex items-center gap-2">
                      ...
                      <p className="text-blue-700 dark:text-blue-300">
                        Hover and click × to remove widgets
                      </p>
                    </div>
                    <div className="bg-white/50 dark:bg-gray-800/50 rounded-lg p-3">
                      <div className="font-semibold text-blue-800 dark:text-blue-200 mb-1 flex items-center gap-2">
                      ...
                      <p className="text-blue-700 dark:text-blue-300">
                        Drag corner handles to resize widgets
                      </p>
                    </div>
                  </div>
                </div>
        ...
        >
          ...
          {isCustomizing && !draggedWidget && (
            <div className="grid-guidelines">
              {Array.from(
                { length: dashboardConfig.gridRows * dashboardConfig.gridCols },
                (_, i) => {
          ...
          <Suspense
            fallback={
              <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30">
                <div className="px-4 py-3 rounded-lg bg-white dark:bg-gray-800 shadow-lg text-gray-700 dark:text-gray-200">
                  Loading widget library...
                </div>
              </div>
            }
          >
