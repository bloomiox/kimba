    ...
    <Card title="Revenue Last 7 Days" className="h-full flex flex-col">
        ...
        {data.map((item, index) => {
          ...
          const barHeight =
            item.value === 0
              ? hasAnyData
                ? 2
                : 8 // Small indicator if other days have data, larger if all zero
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-3 gap-4 text-center mb-6">
          ...
          <div>
            <div className="text-lg font-bold text-gray-900 dark:text-white">{totalSales}</div>
            <div className="text-xs text-gray-500 dark:text-gray-400">
              {t('dashboard.revenueChart.totalSales')}
            </div>
          </div>
          <div>
            <div className="text-lg font-bold text-gray-900 dark:text-white">
              {avgSaleValue > 0
                ? avgSaleValue.toLocaleString(t('language.code'), {
                    style: 'currency',
                    currency: currency,
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400">
              {t('dashboard.revenueChart.avgSaleValue')}
            </div>
          </div>
        </div>
              ...
              return (
                <div key={`avg-${index}`} className="flex-1 flex flex-col items-center gap-1">
                  <div className="text-xs font-medium text-gray-600 dark:text-gray-300">
                    {avgValue > 0
                      ? avgValue.toLocaleString(t('language.code'), {
                          style: 'currency',
                          currency: currency,
                          minimumFractionDigits: 0,
                          maximumFractionDigits: 0,
