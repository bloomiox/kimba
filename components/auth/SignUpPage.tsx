...
interface SignUpPageProps {
  onNavigate: (
    page: 'signin' | 'landing' | 'about' | 'contact' | 'blog' | 'terms' | 'privacy'
  ) => void;
}
      ...
      if (data.user?.id) {
        ...
        if (profileError) {
          ...
          if (profileError.message.includes('row-level security')) {
            setError(
              'Account created successfully, but profile setup needs configuration. Please contact support.'
            );
          } else {
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={t('auth.signUp.salonNamePlaceholder')}
            />
          </div>
          <div>
            <label
