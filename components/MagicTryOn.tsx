  ...
  return (
    <div className="p-4 sm:p-6 flex flex-col h-full">
      <div className="relative w-full aspect-video bg-gray-200 dark:bg-gray-900 rounded-lg overflow-hidden border border-gray-300 dark:border-gray-700 mb-4">
        ...
        {capturedFrame && (
          <img
            src={capturedFrame.base64}
            alt="Captured frame"
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${status === 'GENERATING' ? 'opacity-100' : 'opacity-0'}`}
          />
        )}
        ...
        {resultImage && (
          <img
            ...
            className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${status === 'RESULT' ? 'opacity-100' : 'opacity-0'}`}
          />
        )}
        ...
        {(status === 'GENERATING' || status === 'ERROR') && (
          <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-center p-4">
            ...
            <h2
              className={`text-2xl font-semibold mt-6 ${status === 'ERROR' ? 'text-red-500' : 'text-white'}`}
            >
              {status === 'GENERATING' ? 'Applying Style...' : 'An Error Occurred'}
            </h2>
            <p className="text-gray-300 dark:text-gray-400 mt-2 max-w-sm">
              {status === 'GENERATING' ? 'Hold tight, this will just take a moment.' : error}
            </p>
            {status === 'ERROR' && (
              <button
                onClick={handleTryAgain}
                className="mt-6 px-6 py-2 bg-accent hover:opacity-90 rounded-lg font-semibold text-white transition-all"
              >
                Try Again
              </button>
            )}
      ...
      {status === 'RESULT' ? (
        <div className="flex-grow flex items-center justify-center">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            >
              Try Another Style/Pose
            </button>
            <button
              onClick={handleUseThisLook}
              className="px-8 py-3 bg-accent hover:opacity-90 rounded-lg font-semibold text-white transition-all"
            >
              Use this Look
            </button>
          </div>
        ...
        <div className="flex-grow flex flex-col min-h-0">
          <p className="text-center text-gray-600 dark:text-gray-300 mb-3">
            Click a style to try it on your live image
          </p>
            ...
            <div className="flex gap-3 px-2">
              {hairstyles.map(style => (
                <button
                  ...
                  className="group relative w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed ring-2 ring-transparent hover:ring-accent focus:ring-accent transition-all"
                >
                  <img
                  ...
                  <p className="absolute bottom-0 left-0 w-full p-1 text-white text-xs font-semibold text-center truncate bg-gradient-to-t from-black/80 to-transparent">
                    {style.name}
                  </p>
                </button>
              ))}
      ...
      <div className="mt-4 text-center">
        <button
          onClick={onExit}
          className="text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white transition-colors text-sm"
        >
          Back to Start
        </button>
      </div>
