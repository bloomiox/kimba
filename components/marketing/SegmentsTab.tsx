...
const SegmentsTab: React.FC = () => {
  const { segments, addSegment, updateSegment, deleteSegment, getSegmentClients, t } =
    useSettings();
    ...
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={handleNew}>
          <PlusIcon className="w-5 h-5" /> {t('marketing.segments.new')}
        </Button>
      </div>
        ...
        {segments.length > 0 ? (
          <div className="space-y-3">
            {segments.map(segment => (
              <div
                key={segment.id}
                className="p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg flex justify-between items-center"
              >
                <div>
                  <p className="font-bold">{segment.name}</p>
                  <p className="text-sm text-gray-500">
                    {t('marketing.segments.clients', { count: getSegmentClients(segment).length })}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button onClick={() => handleEdit(segment)} variant="secondary" className="p-2">
                    <EditIcon className="w-4 h-4" />
                  </Button>
                  <Button
                    onClick={() => handleDelete(segment.id)}
                    variant="secondary"
                    className="p-2 hover:bg-red-500/20 hover:text-red-500"
                  >
      ...
      {isModalOpen && (
        <SegmentModal segment={editingSegment} onClose={() => setIsModalOpen(false)} />
      )}
    </div>
  );
};

const SegmentModal: React.FC<{ segment: Segment | null; onClose: () => void }> = ({
  segment,
  onClose,
}) => {
  ...
  const [filters, setFilters] = useState<SegmentFilter[]>(
    segment?.filters || [{ field: 'totalSpent', operator: 'gte', value: 100 }]
  );
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={segment ? t('marketing.segments.editTitle') : t('marketing.segments.newTitle')}
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="name" className="block text-sm font-medium">
            {t('marketing.segments.form.name')}
          </label>
          <input
            type="text"
            id="name"
        ...
        <div className="space-y-3">
          <label className="block text-sm font-medium">
            {t('marketing.segments.form.filters')}
          </label>
          ...
          <Button type="button" onClick={handleAddFilter} variant="secondary" className="text-sm">
            <PlusIcon className="w-4 h-4" /> {t('marketing.segments.form.addFilter')}
          </Button>
        </div>
...
const FilterRow: React.FC<{
  ...
  return (
    <div className="flex gap-2 items-center">
      <select
        value={filter.field}
        onChange={e => onChange({ ...filter, field: e.target.value as SegmentFilter['field'] })}
        className="p-2 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-sm"
      >
        {fields.map(f => (
      ...
      {!isDate && (
        <select
          ...
          onChange={e =>
            onChange({ ...filter, operator: e.target.value as SegmentFilter['operator'] })
          }
          className="p-2 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-sm"
        >
      ...
      <input
        ...
        value={filter.value}
        onChange={e =>
          onChange({ ...filter, value: isDate ? e.target.value : Number(e.target.value) })
        }
