    ...
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={handleNew}>
          <PlusIcon className="w-5 h-5" /> {t('marketing.coupons.new')}
        </Button>
      </div>
        ...
        {coupons.length > 0 ? (
          <div className="space-y-3">
            {coupons.map(coupon => (
              <div
                key={coupon.id}
                className="p-3 bg-gray-100 dark:bg-gray-900/50 rounded-lg flex justify-between items-center"
              >
                <div>
                  <p className="font-bold font-mono text-accent">{coupon.code}</p>
                  <p className="text-sm text-gray-500">
                    {coupon.discountType === 'fixed'
                      ? `${coupon.discountValue.toLocaleString(t('language.code'), { style: 'currency', currency: t('currency.code') })} off`
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className={`${coupon.isActive ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'} relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out`}
                  >
                    <span
                      className={`${coupon.isActive ? 'translate-x-5' : 'translate-x-0'} pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out`}
                    />
                  </button>
                  <Button onClick={() => handleEdit(coupon)} variant="secondary" className="p-2">
...
const CouponModal: React.FC<{ coupon: Coupon | null; onClose: () => void }> = ({
  coupon,
  onClose,
}) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={coupon ? t('marketing.coupons.editTitle') : t('marketing.coupons.newTitle')}
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="code" className="block text-sm font-medium">
            {t('marketing.coupons.form.code')}
          </label>
          <input
            type="text"
            name="code"
            ...
            required
            className="w-full mt-1 p-2 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg uppercase font-mono"
          />
        </div>
        <div className="flex gap-4">
          <div className="flex-grow">
            <label htmlFor="discountType" className="block text-sm font-medium">
              {t('marketing.coupons.form.type')}
            </label>
            <select
              name="discountType"
              id="discountType"
          ...
          <div>
            <label htmlFor="discountValue" className="block text-sm font-medium">
              {t('marketing.coupons.form.value')}
            </label>
            <input
              type="number"
              name="discountValue"
