...
const MainApp: React.FC = () => {
  ...
  const renderView = () => {
    switch (activeView) {
      case 'dashboard':
        return (
          <Dashboard onQuickAction={handleNavigate} savedLookbooks={getCurrentUserLookbooks()} />
        );
      case 'studio':
        return <DesignStudio />;
      case 'clients':
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        <main
          className={`flex-grow p-4 sm:p-6 transition-opacity duration-300 ease-in-out ${contentVisible ? 'opacity-100' : 'opacity-0'}`}
        >
          {renderView()}
        </main>
