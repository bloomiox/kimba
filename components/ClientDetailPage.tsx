...
import { ChevronLeftIcon, UserIcon, CalendarIcon, DesignStudioIcon, MapPinIcon } from './common/Icons';
              ...
              <div className="bg-white dark:bg-gray-800/50 p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700/50 space-y-4 max-h-80 overflow-y-auto">
                {clientAppointments.length > 0 ? clientAppointments.map(app => (
                  <div key={app.id} className="p-3 bg-gray-100 dark:bg-gray-900/50 rounded-lg">
                    <p className="font-semibold">{servicesById[app.serviceId]?.name || t('clients.detail.unknownService')}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {new Date(`${app.date}T${app.time}`).toLocaleString(t('language.code'), { dateStyle: 'medium', timeStyle: 'short' })}
                      {' '}{t('clients.detail.with')}{' '}{hairstylistsById[app.hairstylistId]?.name || t('clients.detail.unknownStylist')}
                    </p>
                  </div>
