...
const CalendarPage: React.FC = () => {
  ...
  const [modalData, setModalData] = useState<{ date: Date; time: string; hairstylistId: string; duration?: number } | null>(null);
  ...
  const { appointments, services, hairstylists, updateAppointmentStatus, getClientById, t } = useSettings();
  ...
  const servicesById = useMemo(() => services.reduce((acc, s) => ({ ...acc, [s.id]: s }), {} as Record<string, typeof services[0]>), [services]);
  ...
  const getHeaderTitle = () => {
    ...
    if (view === 'month') return currentDate.toLocaleString(langCode, { month: 'long', year: 'numeric' });
    if (view === 'day') return currentDate.toLocaleString(langCode, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
    if (view === 'week') {
  ...
  const openAppointmentModal = (date: Date, time: string, hairstylistId: string, duration?: number) => {
  ...
  const renderMultiDayView = (numDays: number) => {
    ...
    if (hairstylists.length === 0) return <div className="text-center p-8 text-gray-500">{t('calendar.noStylist')}</div>;
    if (visibleHairstylists.length === 0) return <div className="text-center p-8 text-gray-500">{t('calendar.noResources')}</div>;
    ...
    return <TimeGrid
        ...
        onStatusChange={(id: string, status: AppointmentStatus) => updateAppointmentStatus(id, status)}
      />
  };
  ...
  return (
    <div className="flex flex-col h-full max-h-full gap-6 animate-fade-in">
      ...
      {isModalOpen && modalData && <AppointmentModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} modalData={modalData} />}
      {viewingAppointment && <AppointmentDetailsModal appointment={viewingAppointment} onClose={() => setViewingAppointment(null)} />}
    </div>
  );
};
