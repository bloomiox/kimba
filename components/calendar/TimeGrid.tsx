...
import { MoreVerticalIcon, CheckIcon, UserIcon, ClockIcon, GripVerticalIcon } from '../common/Icons';
...
const TimeGrid: React.FC<TimeGridProps> = ({ days, hairstylists, appointmentsByDate, servicesById, getClientById, onGridClick, onAppointmentClick, onStatusChange }) => {
    const { updateAppointmentDetails } = useSettings();
    const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
    const resizeRef = useRef<{ appointment: Appointment; startY: number; startDuration: number } | null>(null);
    const [resizingAppointment, setResizingAppointment] = useState<{ id: string; duration: number } | null>(null);
    
    // Drag-to-create state
    const dragCreateRef = useRef<{ startY: number; day: Date; hairstylistId: string; gridCell: HTMLDivElement } | null>(null);
    const [dragSelection, setDragSelection] = useState<{ top: number; height: number; day: Date; hairstylistId: string } | null>(null);
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            newDuration = Math.max(SLOT_INTERVAL_MINUTES, Math.round(newDuration / SLOT_INTERVAL_MINUTES) * SLOT_INTERVAL_MINUTES);
    ...
    // Effect for drag-to-create
     useEffect(() => {
        ...
        const handleMouseUp = (e: MouseEvent) => {
            if (dragSelection && dragSelection.height > 10) { // Min drag height
                ...
                const snappedStartMinutes = Math.round(startMinutes / SLOT_INTERVAL_MINUTES) * SLOT_INTERVAL_MINUTES;
                const snappedDuration = Math.max(SLOT_INTERVAL_MINUTES, Math.round(durationMinutes / SLOT_INTERVAL_MINUTES) * SLOT_INTERVAL_MINUTES);
                ...
                const snappedTotalMinutes = Math.round(totalMinutesFrom8AM / SLOT_INTERVAL_MINUTES) * SLOT_INTERVAL_MINUTES;
    ...
    const STATUS_STYLES: Record<AppointmentStatus, { bg: string; border: string; text: string; lateStripe?: string }> = {
      unconfirmed: { bg: 'bg-blue-500/80 hover:bg-blue-500', border: 'border-blue-300', text: 'text-white' },
      confirmed: { bg: 'bg-green-500/80 hover:bg-green-500', border: 'border-green-300', text: 'text-white' },
      late: { bg: 'bg-orange-500/80 hover:bg-orange-500', border: 'border-orange-300', text: 'text-white', lateStripe: `bg-[repeating-linear-gradient(45deg,rgba(0,0,0,0.1),rgba(0,0,0,0.1)_10px,transparent_10px,transparent_20px)]` },
      cancelled: { bg: 'bg-gray-500/80 hover:bg-gray-500', border: 'border-gray-300', text: 'text-white' },
    };
    ...
    return (
        <div className="flex flex-col h-full bg-gray-100 dark:bg-gray-800/50 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
            <div className="flex sticky top-0 bg-gray-100 dark:bg-gray-800/80 backdrop-blur-sm z-20 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="flex-grow grid" style={{ gridTemplateColumns: `repeat(${days.length * hairstylists.length}, 1fr)` }}>
                    {days.map(day => hairstylists.map(stylist => (
                    <div key={`${getDayString(day)}-${stylist.id}`} className="py-3 text-center border-l border-gray-200 dark:border-gray-700 flex flex-col items-center gap-2 justify-end">
                        {days.length > 1 && (
                            <div className={`text-sm font-semibold ${isSameDay(day, new Date()) ? 'text-accent' : ''}`}>{day.toLocaleString('default', { weekday: 'short' })} {day.getDate()}</div>
                        )}
                        ...
                        <div className="text-sm font-semibold text-gray-800 dark:text-gray-200">{stylist.name}</div>
                    </div>
                    )))}
                </div>
            </div>
                ...
                <div className="flex bg-white dark:bg-gray-900">
                    ...
                    <div className="flex-grow grid grid-cols-1" style={{ gridTemplateColumns: `repeat(${days.length * hairstylists.length}, minmax(140px, 1fr))` }}>
                        {days.map(day => hairstylists.map(stylist => {
                            ...
                            return (
                            <div key={`${dayString}-${stylist.id}`} className="relative border-l border-gray-200 dark:border-gray-700 transition-colors" 
                                onMouseDown={(e) => {
                                    if ((e.target as HTMLElement).closest('button')) return;
                                    const rect = e.currentTarget.getBoundingClientRect();
                                    dragCreateRef.current = { startY: e.clientY - rect.top, day, hairstylistId: stylist.id, gridCell: e.currentTarget };
                                    // A hacky way to pass the cell height to the mouseUp handler
                                    setDragSelection({ top: e.clientY - rect.top, height: 0, day: new Date(rect.height), hairstylistId: stylist.id });
                                }}
                            ...
                            >
                                ...
                                {dragSelection?.hairstylistId === stylist.id && getDayString(dragSelection.day) === dayString && (
                                    <div className="absolute left-1 right-1 bg-accent/20 border-2 border-dashed border-accent rounded-lg opacity-70 pointer-events-none" style={{ top: dragSelection.top, height: dragSelection.height }}></div>
                                )}
                                ...
                                const duration = isResizing ? resizingAppointment.duration : (app.durationOverride || service.duration);
                                        ...
                                        <div className="flex justify-between items-start w-full">
                                            ...
                                            <div className="relative appointment-dropdown-container">
                                                ...
                                                {activeDropdown === app.id && (
                                                    ...
                                                    {(['unconfirmed', 'confirmed', 'late'] as AppointmentStatus[]).map(status => (
                                                        <button key={status} onClick={(e) => { e.stopPropagation(); onStatusChange(app.id, status); setActiveDropdown(null); }} className="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 capitalize">Mark as {status}</button>
                                                    ))}
                                                    </div>
                                                )}
                                            </div>
