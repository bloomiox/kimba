...
const getAvailableSlots = (
  ...
  const servicesById = allServices.reduce(
    (acc, s) => ({ ...acc, [s.id]: s }),
  ...
  const partialTimeOffIntervals = (hairstylist.timeOff || [])
    .filter(
      timeOff =>
        !timeOff.isFullDay && dateString >= timeOff.startDate && dateString <= timeOff.endDate
    )
    .map(timeOff => {
  ...
  for (
    ...
    slotTime < workEnd;
    slotTime.setMinutes(slotTime.getMinutes() + SLOT_INTERVAL)
  ) {
...
const BookingForm: React.FC<BookingFormProps> = ({ onClose }) => {
  ...
  const topLevelGroups = useMemo(
    () => serviceGroups.filter(g => g.parentId === null),
    [serviceGroups]
  );
  const hasOrphanServices = useMemo(() => services.some(s => s.parentId === null), [services]);

  const allCategories = useMemo(() => {
    const categories: (ServiceGroup | { id: string; name: string; parentId: string | null })[] = [
      ...topLevelGroups,
    ];
    if (hasOrphanServices) {
      categories.push({
        id: 'root_services',
        name: t('booking.step1.generalServices'),
        parentId: null,
      });
    }
  ...
  const hairstylistAppointments = useMemo(() => {
    ...
    return appointments.filter(
      a => a.hairstylistId === selectedHairstylist.id && a.date === dateString
    );
      ...
      <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-8 text-center animate-fade-in">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">
            {t('booking.confirmed.subtitle', { salonName })}
          </p>

          <div
            className={`bg-gradient-to-r ${mapToAccentColor('from-accent-50 to-accent-100 dark:from-accent-900/20 dark:to-accent-800/20')} p-6 rounded-lg text-left space-y-3 mb-6`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-8 h-8 ${mapToAccentColor('bg-accent-500')} rounded-full flex items-center justify-center`}
              >
                <span className="text-white text-sm font-bold">✓</span>
              </div>
              <div>
                <p
                  className={`font-semibold ${mapToAccentColor('text-accent-900 dark:text-accent-100')}`}
                >
                  {selectedService?.name}
                </p>
                <p
                  className={`text-sm ${mapToAccentColor('text-accent-700 dark:text-accent-300')}`}
                >
            ...
            <div className="flex items-center gap-3">
              <UserIcon
                className={`w-8 h-8 p-1.5 ${mapToAccentColor('bg-accent-500')} text-white rounded-full`}
              />
              <div>
                <p
                  className={`font-semibold ${mapToAccentColor('text-accent-900 dark:text-accent-100')}`}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  {new Date(
                    formatDateForStorage(selectedDate) + 'T' + selectedTime
                  ).toLocaleDateString(t('language.code'), {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
                {allCategories.map(g => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 ${mapToAccentColor('bg-accent-100 dark:bg-accent-900/30')} rounded-lg flex items-center justify-center ${mapToAccentColor('group-hover:bg-accent-500')} transition-colors`}
                      >
                        <span className="text-2xl group-hover:text-white">✂️</span>
                      </div>
                      <div>
                        <p
                          className={`font-semibold text-lg text-gray-900 dark:text-white ${mapToAccentColor('group-hover:text-accent-600 dark:group-hover:text-accent-400')}`}
                        >
                          {g.name}
                        </p>
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        <div
                          className={`text-xl font-bold ${mapToAccentColor('text-accent-600 dark:text-accent-400')}`}
                        >
                          {s.price.toLocaleString(t('language.code'), {
                            style: 'currency',
                            currency: 'USD',
                          })}
                        </div>
          ...
          {step === 3 && (
            <div className="animate-fade-in">
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Select your preferred hairstylist
                </p>
              </div>
                        ...
                        {h.skills.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {h.skills.slice(0, 2).map(skill => (
                              <span
                                key={skill.id}
                                className={`px-2 py-1 ${mapToAccentColor('bg-accent-100 dark:bg-accent-900/30 text-accent-700 dark:text-accent-300')} text-xs rounded-full`}
                              >
                                {skill.name}
                              </span>
                            ))}
              ...
              <div className="max-w-4xl mx-auto">
                <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="lg:w-1/3">
                    ...
                    <div
                      className={`mt-4 p-4 ${mapToAccentColor('bg-accent-50 dark:bg-accent-900/20')} rounded-lg`}
                    >
                      <p
                        className={`text-sm ${mapToAccentColor('text-accent-700 dark:text-accent-300')} font-medium`}
                      >
                        Selected Date
                      </p>
                      <p
                        className={`text-lg font-semibold ${mapToAccentColor('text-accent-900 dark:text-accent-100')}`}
                      >
                        ...
                        availableSlots.map(slot => (
                          ...
                          >
                            <div className="text-center">
                              <div
                                className={`text-lg font-bold text-gray-900 dark:text-white ${mapToAccentColor('group-hover:text-accent-600 dark:group-hover:text-accent-400')}`}
                              >
              ...
              <div className="max-w-4xl mx-auto">
                <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="relative">
                          ...
                          {isClientListVisible && clientDetails.name && (
                            <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                              {clients
                                .filter(c =>
                                  c.name.toLowerCase().includes(clientDetails.name.toLowerCase())
                                )
                                .slice(0, 5)
                                .map(c => (
                                  <button
                  ...
                  <div className="lg:w-1/2">
                    ...
                    <div
                      className={`bg-gradient-to-r ${mapToAccentColor('from-accent-50 to-accent-100 dark:from-accent-900/20 dark:to-accent-800/20')} p-6 rounded-xl space-y-4`}
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 ${mapToAccentColor('bg-accent-500')} rounded-full flex items-center justify-center`}
                        >
                          <span className="text-white text-lg">✂️</span>
                        </div>
                        <div>
                          <p
                            className={`font-semibold ${mapToAccentColor('text-accent-900 dark:text-accent-100')} text-lg`}
                          >
                            {selectedService?.name}
                          </p>
                          <p
                          ...
                          >
                            {selectedService?.duration} {t('common.minutes')} •{' '}
                            {selectedService?.price.toLocaleString(t('language.code'), {
                              style: 'currency',
                              currency: 'USD',
                            })}
                          </p>
                      ...
                      <div className="flex items-center gap-4">
                        <UserIcon
                          className={`w-12 h-12 p-2 ${mapToAccentColor('bg-accent-500')} text-white rounded-full`}
                        />
                        <div>
                          <p
                            className={`font-semibold ${mapToAccentColor('text-accent-900 dark:text-accent-100')}`}
                          ...
                          <p
                            className={`text-sm ${mapToAccentColor('text-accent-700 dark:text-accent-300')} capitalize`}
                          >
