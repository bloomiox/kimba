    ...
    <div className="p-4 sm:p-8 h-full flex flex-col animate-fade-in">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
          Step 4: Your Final Lookbook
        </h2>
        <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-xl mx-auto">
          Here is your refined hairstyle from multiple angles. You can now save these images or
          start a new session.
        </p>
      </div>
      ...
      <div className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="w-full">
          <h3 className="text-xl font-semibold text-center lg:text-left mb-4 text-gray-800 dark:text-gray-200">
            Different Angles
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {allViews.map(view => (
              <div key={view.view} className="flex flex-col items-center gap-2">
                <img
                  src={view.src}
                  alt={`Angle view: ${view.view}`}
                  className="w-full aspect-square object-cover rounded-lg shadow-lg"
                />
                <p className="font-semibold text-gray-600 dark:text-gray-300">{view.view} View</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="text-center mt-8 flex flex-wrap justify-center items-center gap-4">
        ...
        <button className="px-6 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white transition-colors">
          Download Lookbook
        </button>
      </div>
    </div>
