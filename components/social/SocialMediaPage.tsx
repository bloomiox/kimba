...
const SocialMediaPage: React.FC = () => {
  ...
  const handleSelectLookbook = (lookbook: Lookbook) => {
    ...
    setCaption(
      `Another amazing transformation at ${salonName}! ✨ Swipe to see the before. What do you think of this new look for ${clientFirstName}?\n\n#hairstyles #hairtransformation #${salonName.replace(/\s/g, '').toLowerCase()}`
    );
  };
      ...
      <Card title={t('social.connections.title')}>
        <div className="space-y-4">
          {socialPlatforms.map(p => (
            <div
              key={p.id}
              className="flex items-center justify-between p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {p.handle
                      ? t('social.connections.connectedAs', { username: p.handle })
                      : t('social.connections.notConnected')}
                  </p>
                </div>
              </div>
              {p.handle ? (
                <Button onClick={() => disconnectSocialMedia(p.id)} variant="secondary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="mt-4">
                ...
                <div className="grid grid-cols-3 gap-2 max-h-60 overflow-y-auto">
                  {clientLookbooks.map(lb => (
                    <button
                      key={lb.id}
                      onClick={() => handleSelectLookbook(lb)}
                      className={`relative aspect-square rounded-lg overflow-hidden ring-2 ${selectedLookbook?.id === lb.id ? 'ring-accent' : 'ring-transparent'}`}
                    >
                      <img
          ...
          <div>
            {selectedLookbook ? (
              <div>
                <img
                  src={selectedLookbook.finalImage.src}
                ...
                <Button onClick={handlePost} className="w-full mt-4">
                  {t('social.creator.postNow')}
                </Button>
              </div>
            ) : (
