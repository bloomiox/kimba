...
import { ChevronLeftIcon, ChevronRightIcon, FilterIcon, MoreVerticalIcon, CheckIcon } from './common/Icons';
...
const CalendarPage: React.FC = () => {
  ...
  const [modalData, setModalData] = useState<{ date: Date; time: string; hairstylistId: string } | null>(null);
  const [viewingAppointment, setViewingAppointment] = useState<Appointment | null>(null);
  const [isFilterPanelOpen, setIsFilterPanelOpen] = useState(false);
  const [visibleHairstylistIds, setVisibleHairstylistIds] = useState<string[]>([]);
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);

  const { appointments, services, hairstylists, updateAppointmentStatus, updateAppointmentDetails, getClientById } = useSettings();
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (activeDropdown && !(event.target as HTMLElement).closest('.appointment-dropdown-container')) {
    ...
    return (
      <div className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-30 p-4">
        ...
        <div className="flex justify-between mb-2">
          <button onClick={handleSelectAll} className="text-xs text-accent font-semibold">Select All</button>
          <button onClick={handleSelectNone} className="text-xs text-accent font-semibold">Select None</button>
        </div>
        <div className="space-y-2 max-h-60 overflow-y-auto">
          {hairstylists.map(stylist => (
            <label key={stylist.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
              <input
                type="checkbox"
              ...
              <div>
                ...
                <span className="block text-xs text-gray-500 dark:text-gray-400 capitalize">{stylist.type}</span>
              </div>
            </label>
          ))}
        </div>
  ...
  const renderMonthView = () => {
    ...
    const calendarGrid = Array.from({ length: lastDay.getDate() }, (_, i) => new Date(firstDay.getFullYear(), firstDay.getMonth(), i + 1));
    const paddingDays = Array.from({ length: firstDay.getDay() }).map((_, i) => new Date(firstDay.getFullYear(), firstDay.getMonth(), i - firstDay.getDay() + 1));
    ...
    return (
      <div className="grid grid-cols-7 gap-px overflow-hidden rounded-lg bg-gray-200 dark:bg-gray-700 border border-gray-200 dark:border-gray-700">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => <div key={day} className="py-2 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800">{day}</div>)}
        {[...paddingDays, ...calendarGrid].map((day, index) => {
          const dateString = getDayString(day);
          const isCurrentMonth = day.getMonth() === currentDate.getMonth();
          return (
            <button key={index} onClick={() => { setView('day'); setCurrentDate(day); }} className={`relative min-h-[120px] p-2 text-left transition-colors flex flex-col ${isCurrentMonth ? 'bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800' : 'bg-gray-50 dark:bg-gray-900/50 text-gray-400'}`}>
              <span className={`text-sm font-semibold ${isSameDay(day, new Date()) ? 'bg-accent text-white rounded-full w-6 h-6 flex items-center justify-center' : ''}`}>{day.getDate()}</span>
              {appointmentsByDate[dateString] && (
                <div className="mt-1 flex-grow overflow-hidden space-y-1">
                  {appointmentsByDate[dateString].slice(0, 3).map(app => (
                    <div key={app.id} className="w-full text-xs p-1 bg-accent/20 text-accent-800 dark:text-accent-200 rounded truncate">{getClientById(app.clientId)?.name}</div>
                  ))}
                </div>
              )}
            </button>
  ...
  const TimeGrid: React.FC<{ days: Date[]; hairstylists: Hairstylist[] }> = ({ days, hairstylists }) => {
    const hours = Array.from({ length: 13 }, (_, i) => i + 8); // 8am to 8pm
    const STATUS_STYLES: Record<AppointmentStatus, { bg: string; border: string; text: string; lateStripe?: string }> = {
      ...
      late: { bg: 'bg-orange-500', border: 'border-orange-300', text: 'text-white', lateStripe: `bg-[repeating-linear-gradient(45deg,rgba(0,0,0,0.1),rgba(0,0,0,0.1)_10px,transparent_10px,transparent_20px)]` },
      cancelled: { bg: 'bg-gray-500', border: 'border-gray-300', text: 'text-white' },
    };

    const handleGridClick = (e: React.MouseEvent<HTMLDivElement>, day: Date, hairstylistId: string) => {
      ...
      openAppointmentModal(day, `${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`, hairstylistId);
    };
    ...
    return (
      <div className="flex-grow flex bg-white dark:bg-gray-900">
        <div className="w-16 flex-shrink-0 text-right pr-2 text-xs text-gray-400">
          {hours.map(hour => <div key={hour} className="h-24 -mt-3 pt-3">{`${hour % 12 === 0 ? 12 : hour % 12} ${hour < 12 ? 'AM' : 'PM'}`}</div>)}
        </div>
        <div className="flex-grow grid grid-cols-1" style={{ gridTemplateColumns: `repeat(${days.length * hairstylists.length}, minmax(140px, 1fr))` }}>
          {days.map(day => hairstylists.map(stylist => {
            const dayString = getDayString(day);
            const dayAppointments = (appointmentsByDate[dayString] || []).filter(a => a.hairstylistId === stylist.id);
            return (
              <div key={`${dayString}-${stylist.id}`} className="relative border-l border-gray-200 dark:border-gray-700" onClick={(e) => handleGridClick(e, day, stylist.id)}>
                {hours.map(h => <div key={h} className="h-24 border-t border-gray-200 dark:border-gray-700"></div>)}
                {dayAppointments.map(app => {
                  ...
                  return (
                    <button key={app.id} onClick={(e) => { e.stopPropagation(); setViewingAppointment(app); }} className={`absolute left-1 right-1 p-2 rounded-lg border-l-4 z-10 shadow-lg overflow-hidden text-xs text-left ${style.bg} ${style.border} ${style.text}`} style={{ top: `${top}px`, height: `${height}px`, minHeight: '24px' }}>
                      <div className={`absolute inset-0 ${style.lateStripe || ''}`}></div>
                      <div className="relative flex justify-between items-start">
                        <div>
                          <p className="font-bold flex items-center gap-1.5">
                            {app.status === 'confirmed' && <CheckIcon className="w-4 h-4 flex-shrink-0" />}
                            {client.name}
                          </p>
                        ...
                        <div className="relative appointment-dropdown-container">
                          <button onClick={(e) => { e.stopPropagation(); setActiveDropdown(app.id === activeDropdown ? null : app.id); }} className="p-1 -mr-1 -mt-1 rounded-full hover:bg-black/20 flex-shrink-0">
                          ...
                          {activeDropdown === app.id && (
                            <div className="absolute top-full right-0 mt-1 w-36 bg-white dark:bg-gray-700 rounded-md shadow-xl z-20 overflow-hidden">
                              {(['unconfirmed', 'confirmed', 'late', 'cancelled'] as AppointmentStatus[]).map(status => (
                                <button key={status} onClick={(e) => { e.stopPropagation(); updateAppointmentStatus(app.id, status); setActiveDropdown(null); }} className="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 capitalize">Mark as {status}</button>
                              ))}
                            </div>
                          )}
                        </div>
  ...
  const renderMultiDayView = (numDays: number) => {
    ...
    const viewDays = Array.from({ length: numDays }).map((_, i) => new Date(startDay.getFullYear(), startDay.getMonth(), startDay.getDate() + i));

    if (hairstylists.length === 0) return <div className="text-center p-8 text-gray-500">Please add a hairstylist in Settings to begin.</div>;
    if (visibleHairstylists.length === 0) return <div className="text-center p-8 text-gray-500">No resources selected. Use the filter to select hairstylists.</div>;
    ...
    return (
      <div className="flex flex-col h-full bg-gray-100 dark:bg-gray-800/50 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
        <div className="flex sticky top-0 bg-gray-100 dark:bg-gray-800/80 backdrop-blur-sm z-20">
          ...
          <div className="flex-grow grid" style={{ gridTemplateColumns: `repeat(${numDays * visibleHairstylists.length}, 1fr)` }}>
            {viewDays.map(day => visibleHairstylists.map(stylist => (
              <div key={`${getDayString(day)}-${stylist.id}`} className="py-2 text-center border-l border-gray-200 dark:border-gray-700">
                {numDays > 1 && <div className={`font-semibold ${isSameDay(day, new Date()) ? 'text-accent' : ''}`}>{day.toLocaleString('default', { weekday: 'short' })} {day.getDate()}</div>}
    ...
    <div className="flex flex-col h-full max-h-full gap-6 animate-fade-in">
      <header className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center bg-gray-200 dark:bg-gray-900 p-1 rounded-lg">
          {(['month', 'week', 'day'] as CalendarView[]).map(v => <button key={v} onClick={() => setView(v)} className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${view === v ? 'bg-white dark:bg-gray-700 text-accent shadow' : 'text-gray-600 dark:text-gray-300'}`}>{v.charAt(0).toUpperCase() + v.slice(1)}</button>)}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => handleNavigate('today')} className="px-4 py-1.5 text-sm font-semibold rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700">Today</button>
