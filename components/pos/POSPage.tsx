...
const POSPage: React.FC = () => {
  ...
  const handleCheckout = (paymentMethod: PaymentMethod) => {
    ...
    addSale({
      ...
      discount: appliedCoupon
        ? { amount: calculations.discountAmount, reason: appliedCoupon.code }
        : null,
  ...
  const ServiceTree: React.FC<{ parentId: string | null; level: number }> = ({
    ...
    const childGroups = serviceGroups
      ...
      <div className={`${level > 0 ? 'ml-4' : ''}`}>
        ...
        {childServices.map(service => (
          <button
            ...
            className="w-full text-left p-2 my-1 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-md transition-colors"
          >
            {service.name} -{' '}
        ...
        <div className="flex-grow flex flex-col p-4 bg-white dark:bg-gray-800/50 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700/50">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <p className="font-semibold">
                      {item.price.toLocaleString(langCode, { style: 'currency', currency })}
                    </p>
                    <button
          ...
          <div className="flex-shrink-0 pt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between items-center">
              ...
              {appliedCoupon ? (
                <div className="text-green-600 dark:text-green-400 font-semibold flex items-center gap-2">
                  <span>
                    -{' '}
                    {calculations.discountAmount.toLocaleString(langCode, {
                      style: 'currency',
                      currency,
                    })}
                  </span>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    placeholder={t('pos.coupon.code')}
                    className="w-24 p-1 text-sm bg-gray-100 dark:bg-gray-700 rounded-md uppercase"
                  />
                  <button
                    onClick={handleApplyCoupon}
                    className="text-xs px-2 py-1 rounded bg-gray-200 dark:bg-gray-600 hover:bg-gray-300"
                  >
                    {t('pos.summary.apply')}
                  </button>
                </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-xs px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-600 hover:bg-gray-300"
                >
                ...
                <input
                  ...
                  className="w-16 p-1 text-right bg-gray-100 dark:bg-gray-700 rounded-md"
                  placeholder="0.00"
                />
              </div>
            </div>
            <div className="flex justify-between font-bold text-lg pt-2 border-t dark:border-gray-600">
              ...
              <span>
                {calculations.total.toLocaleString(langCode, { style: 'currency', currency })}
              </span>
            </div>
          </div>
          <button
            onClick={() => setIsPaymentModalOpen(true)}
            disabled={cart.length === 0 || !selectedHairstylistId}
            className="w-full mt-4 py-3 bg-accent text-white font-bold rounded-lg hover:opacity-90 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
          >
            {t('pos.checkout')}
          </button>
        </div>
      ...
      <div className="w-full lg:w-3/5 xl:w-2/3 p-4 bg-white dark:bg-gray-800/50 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700/50 flex flex-col">
        <div className="flex-shrink-0 flex border-b dark:border-gray-700 mb-2">
          <button
            ...
            className={`px-4 py-2 font-semibold ${catalogTab === 'services' ? 'text-accent border-b-2 border-accent' : 'text-gray-500'}`}
          >
            {t('pos.catalog.services')}
          </button>
          <button
            onClick={() => setCatalogTab('products')}
            className={`px-4 py-2 font-semibold ${catalogTab === 'products' ? 'text-accent border-b-2 border-accent' : 'text-gray-500'}`}
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {products.map(product => (
                <button
                  ...
                  className="p-3 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-md transition-colors text-left"
                >
