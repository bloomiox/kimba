    ...
    <>
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4 text-sm">
          Select a hairstyle to get started.
        </p>
          ...
          {hairstyles.map(style => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black/80 to-transparent">
                <p className="text-white text-xs sm:text-sm font-semibold text-center truncate">
                  {style.name}
                </p>
              </div>
            </button>
      ...
      {activeBaseStyle?.variations && (
        <div className="mt-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">
            Variations for {activeBaseStyle.name}
          </h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            <button
              onClick={() => onSelectStyle(activeBaseStyle)}
              className={`group relative aspect-square rounded-lg overflow-hidden transition-all duration-200 ${
                selectedStyle?.id === activeBaseStyle.id
                  ? 'ring-4 ring-accent'
                  : 'ring-2 ring-transparent hover:ring-accent'
              }`}
            >
              <img
                src={activeBaseStyle.previewUrl}
                alt={activeBaseStyle.name}
      ...
      <div className="mt-6">
        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-1">
          2. Refine Your Style (Optional)
        </h3>
        <p className="text-gray-500 dark:text-gray-400 mb-4 text-sm">
          Add more details like color, length, or other specifics.
        </p>
        <textarea
