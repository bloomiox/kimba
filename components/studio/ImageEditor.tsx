...
const ImageEditor: React.FC<{
  ...
  const COLORS = useMemo(
    () => [
      {
        name: t('studio.imageEditor.colors.blonde'),
        prompt: 'in a platinum blonde color',
        color: '#F2E5BF',
      },
      {
        name: t('studio.imageEditor.colors.brunette'),
        prompt: 'in a rich brunette color',
        color: '#5C3A21',
      },
      {
        name: t('studio.imageEditor.colors.black'),
        prompt: 'in a jet black color',
        color: '#222222',
      },
      {
        name: t('studio.imageEditor.colors.red'),
        prompt: 'in a vibrant red color',
        color: '#C93C3C',
      },
      {
        name: t('studio.imageEditor.colors.pastelPink'),
        prompt: 'in a pastel pink color',
        color: '#F4C2C2',
      },
      {
        name: t('studio.imageEditor.colors.silver'),
        prompt: 'in a metallic silver color',
        color: '#C0C0C0',
      },
    ],
    [t]
  ...
  const STYLES = useMemo(
    () => [
      ...
      {
        name: t('studio.imageEditor.styles.straighter'),
        prompt: 'but make it perfectly straight and sleek',
      },
      { name: t('studio.imageEditor.styles.addBangs'), prompt: 'with sharp, blunt-cut bangs' },
      {
        name: t('studio.imageEditor.styles.moreVolume'),
        prompt: 'with significantly more volume and texture',
      },
        ...
        <div className="w-full lg:w-1/2 h-1/2 lg:h-full flex flex-col bg-white/50 dark:bg-gray-800/50 p-6 rounded-lg overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-4 items-center gap-2">
                <label htmlFor="saturation" className="text-sm font-medium col-span-1">
                  {t('studio.imageEditor.saturation')}
                </label>
                <input
                  id="saturation"
                  type="range"
          ...
          <div>
            <h3 className="text-lg font-semibold text-accent mb-3">
              {t('studio.imageEditor.refinements')}
            </h3>
          ...
          <div className="mt-6 flex-grow flex flex-col">
            <h3 className="text-base font-semibold text-gray-700 dark:text-gray-300 mb-2">
              {t('studio.imageEditor.customDetails')}
            </h3>
            <textarea
              value={prompt}
