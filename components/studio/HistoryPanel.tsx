  ...
  return (
    <>
      <aside className="w-56 flex-shrink-0 bg-gray-100/50 dark:bg-gray-900/50 p-4 rounded-2xl flex flex-col">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 text-center">{t('studio.history.title')}</h3>
        <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-3">
          {images.map((image, index) => (
            ...
            >
              <img src={image.src} alt={'hairstyleName' in image ? image.hairstyleName : image.view} className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2">
                <p className="text-white text-xs font-semibold text-center">{'hairstyleName' in image ? image.hairstyleName : t('studio.finalLook.view', { view: image.view })}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* Fullscreen Modal */}
      {fullscreenImage && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-lg z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setFullscreenImage(null)}>
          <div className="relative w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
            <img src={fullscreenImage.src} alt={'hairstyleName' in fullscreenImage ? fullscreenImage.hairstyleName : fullscreenImage.view} className="w-full h-auto object-contain rounded-xl shadow-2xl" />
            <button
