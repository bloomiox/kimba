...
const DesignStudio: React.FC = () => {
    ...
    try {
      const generationPromises = stylesToGenerateFrom.map(styleInfo => {
        ...
        return generateHairstyle(
          ...
          isCustomStyle
            ? `A high-quality photo of a person with a ${styleInfo.name} hairstyle.`
            : styleInfo.prompt,
      ...
      const errorMessage =
        err instanceof Error ? err.message : t('studio.generating.error.generic');
      changeStudioState(StudioState.ERROR, () => {
        setError(errorMessage);
      });
    }
  }, [
    lookbookPieces.userImage,
    changeStudioState,
    incrementImageCount,
    t,
    customHairstyles,
  ...
  const handleGenerateFinalViews = useCallback(async () => {
    ...
    try {
      ...
      const viewPrompts = [
        {
          view: 'Side',
          promptModifier: ', side view, from the left',
          reference: referenceStyle?.leftView,
        },
        {
          view: 'Angled',
          promptModifier: ', 45-degree angle view',
          reference: referenceStyle?.rightView ?? referenceStyle?.leftView,
        },
  ...
  const renderContent = () => {
    switch (studioState) {
      ...
      case StudioState.Step4_GeneratingFinal:
        const messages = {
          [StudioState.Step2_GeneratingInitial]: {
            title: t('studio.generating.initial.title'),
            description: t('studio.generating.initial.subtitle'),
          },
          [StudioState.Step4_GeneratingFinal]: {
            title: t('studio.generating.final.title'),
            description: t('studio.generating.final.subtitle'),
          },
        };
