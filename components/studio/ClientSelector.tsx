...
const ClientSelector: React.FC<ClientSelectorProps> = ({ onClientSelected }) => {
    ...
    return (
        <div className="p-8 flex flex-col items-center justify-center h-full">
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 text-center">{t('studio.clientSelector.title')}</h2>
            ...
            <div className="w-full max-w-lg">
                { !showNewClientForm ? (
                    <>
                        ...
                        <div className="space-y-3 max-h-64 overflow-y-auto">
                            {filteredClients.map(client => (
                                <button key={client.id} onClick={() => onClientSelected(client.id)} className="w-full flex items-center gap-4 p-4 text-left bg-gray-100/50 dark:bg-gray-700/50 hover:bg-accent/10 rounded-lg transition-colors">
                    ...
                    <form onSubmit={handleAddNewClient} className="space-y-4">
                        <h3 className="font-bold text-lg text-center">{t('studio.clientSelector.newClientDetails')}</h3>
                        <Input type="text" name="name" placeholder={t('studio.clientSelector.fullName')} required />
                        <Input type="email" name="email" placeholder={t('studio.clientSelector.email')} required />
                        <div className="flex justify-end gap-3">
                            <Button type="button" onClick={() => setShowNewClientForm(false)} variant="secondary">{t('common.cancel')}</Button>
