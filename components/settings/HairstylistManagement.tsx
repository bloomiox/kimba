...
const HairstylistManagement: React.FC = () => {
  ...
  const handleDelete = (hairstylistId: string) => {
    if (
      window.confirm(
        'Are you sure you want to delete this hairstylist? This may affect existing appointments.'
      )
    ) {
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <HairstylistModal hairstylist={editingHairstylist} onClose={() => setIsModalOpen(false)} />
      )}
    </>
  );
};
