...
const HairstylistModal: React.FC<HairstylistModalProps> = ({ hairstylist, onClose }) => {
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title={`${hairstylist ? 'Edit' : 'Add'} Hairstylist/Resource`}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="name" className="block text-sm font-medium">Name</label>
                    <Input type="text" name="name" id="name" defaultValue={hairstylist?.name} required className="mt-1"/>
                </div>
                <div>
