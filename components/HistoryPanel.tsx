  ...
  return (
    <>
      <aside className="w-56 flex-shrink-0 bg-gray-100/50 dark:bg-gray-900/50 p-4 rounded-2xl flex flex-col">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 text-center">Session History</h3>
          ...
          {images.map((image, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2">
                <p className="text-white text-xs font-semibold text-center">{'hairstyleName' in image ? image.hairstyleName : `${image.view} View`}</p>
              </div>
            </button>
          ))}
        </div>
