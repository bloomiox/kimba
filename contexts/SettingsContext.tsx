...
interface SettingsContextType extends UserSettings {
  ...
  salonName: string;
  setSalonName: (name: string) => Promise<void>;
  ...
  updatePreferences: (values: Partial<PreferencesState>) => void;
  updateBusinessSettings: (values: Partial<BusinessState>) => void;
  ...
  connectSocialMedia: (
    platform: keyof SocialMediaConnections,
...
const DEFAULT_SETTINGS: UserSettings = {
  ...
  dashboardConfiguration: JSON.parse(
    JSON.stringify(DEFAULT_DASHBOARD_CONFIG)
  ) as DashboardConfiguration,
  hasCompletedOnboarding: false,
};

export const SettingsProvider: React.FC<{ children: ReactNode; publicSalonId?: string }> = ({
  children,
  publicSalonId,
}) => {
  ...
  const fetchProfileData = useCallback(async (userOrId: User | string) => {
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        // PGRST116 means 0 rows found, which is not a fatal error
        throw error;
      }

      // If no profile exists (e.g., new signup, DB delay), create a default one.
      if (!profileData) {
        logger.warn('User profile not found for user_id:', userId, 'Creating a default profile.');
      ...
      logger.debug('Database settings:', dbSettings);
      ...
      const finalSettings: UserSettings = {
        ...
        studioInitialGenerations:
          dbSettings.studioInitialGenerations ?? DEFAULT_SETTINGS.studioInitialGenerations,
        ...
        dashboardConfiguration:
          dbSettings.dashboardConfiguration ??
          (JSON.parse(JSON.stringify(DEFAULT_DASHBOARD_CONFIG)) as DashboardConfiguration),
        hasCompletedOnboarding:
          dbSettings.hasCompletedOnboarding ?? DEFAULT_SETTINGS.hasCompletedOnboarding,
      };
    ...
    if (publicSalonId) {
      ...
      supabase.auth.getSession().then(({ data: { session } }) => {
        ...
        if (session?.user) {
          ...
          loadTranslations(DEFAULT_SETTINGS.language)
            .then(() => {
              setTranslationsLoaded(true);
            })
            .catch(error => {
              logger.error('Failed to load default translations:', error);
  ...
  useEffect(() => {
      ...
      .on(
        ...
        async payload => {
          if (payload.eventType === 'INSERT') {
            ...
            setProfile(p => ({
              ...p,
              appointments: (p.appointments || []).filter(a => a.id !== payload.old.id),
            }));
          }
        }
      )
      .subscribe();
    const lookbookSubscription = supabase
      .channel('public:lookbooks')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'lookbooks', filter: `user_id=eq.${user.id}` },
        async payload => {
          if (payload.eventType === 'INSERT') {
            ...
            setProfile(p => ({
              ...p,
              lookbooks: (p.lookbooks || []).map(l => (l.id === payload.new.id ? payload.new : l)),
            }));
          } else if (payload.eventType === 'DELETE') {
            setProfile(p => ({
              ...p,
              lookbooks: (p.lookbooks || []).filter(l => l.id !== payload.old.id),
            }));
          }
        }
      )
      ...
      .on(
        ...
        async payload => {
          if (payload.eventType === 'INSERT') {
            setProfile(p => ({
              ...p,
              settings: { ...p.settings, services: [...(p.settings?.services || []), payload.new] },
            }));
          } else if (payload.eventType === 'UPDATE') {
            setProfile(p => ({
              ...p,
              settings: {
                ...p.settings,
                services: (p.settings?.services || []).map(s =>
            ...
            setProfile(p => ({
              ...
              settings: {
                ...p.settings,
                services: (p.settings?.services || []).filter(s => s.id !== payload.old.id),
              },
            }));
          }
        }
    ...
    const hairstylistsSubscription = supabase
      .channel('public:hairstylists')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'hairstylists', filter: `user_id=eq.${user.id}` },
        async payload => {
          if (payload.eventType === 'INSERT') {
            const hairstylist = {
              id: payload.new.id,
            ...
            setProfile(p => ({
              ...
              settings: {
                ...p.settings,
                hairstylists: [...(p.settings?.hairstylists || []), hairstylist],
              },
            }));
          }
        }
      ...
      .on(
        ...
        async payload => {
          if (payload.eventType === 'INSERT') {
            ...
            setProfile(p => ({
              ...p,
              settings: { ...p.settings, products: [...(p.settings?.products || []), product] },
            }));
          }
        }
      )
    ...
    const serviceGroupsSubscription = supabase
      .channel('public:service_groups')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'service_groups', filter: `user_id=eq.${user.id}` },
        async payload => {
          if (payload.eventType === 'INSERT') {
            const serviceGroup = {
              id: payload.new.id,
            ...
            setProfile(p => ({
              ...
              settings: {
                ...p.settings,
                serviceGroups: (p.settings?.serviceGroups || []).map(sg =>
                  sg.id === serviceGroup.id ? serviceGroup : sg
                ),
              },
            }));
          } else if (payload.eventType === 'DELETE') {
            setProfile(p => ({
              ...
              settings: {
                ...p.settings,
                serviceGroups: (p.settings?.serviceGroups || []).filter(
                  sg => sg.id !== payload.old.id
                ),
              },
            }));
          }
  ...
  const updateSettings = async (newSettings: Partial<UserSettings>) => {
    if (!user) {
      logger.error('updateSettings: User not authenticated');
      ...
      logger.debug('Updating settings locally (no user):', newSettings);
      setProfile(p => ({ ...p, settings: updatedSettings }));
      return;
    }

    logger.debug('updateSettings called with:', newSettings);
    const currentSettings = profile.settings || DEFAULT_SETTINGS;
    const updatedSettings = { ...currentSettings, ...newSettings };
    ...
    if (error) {
      ...
      logger.debug('New settings in database:', updatedSettings);
    }
  };
  ...
  useEffect(() => {
    logger.debug('Theme and accent color useEffect triggered');
    logger.debug('Current profile settings:', profile.settings);
    logger.debug('DEFAULT_SETTINGS:', DEFAULT_SETTINGS);
    ...
    logger.debug('Applied theme class:', settings.theme);

    // Apply accent color - use custom color if selected, otherwise use preset
    const accentColorValue =
      settings.accentColor === 'custom' && settings.customAccentColor
    ...
    logger.debug('Setting accent color to:', accentColorValue, 'from', settings.accentColor);
    ...
    if (accentColorValue) {
      ...
      Object.entries(shades).forEach(([shade, value]) => {
        ...
        logger.debug(`Setting --accent-${shade} to ${value}`);
      });
    }
    ...
    if (settings.language) {
      ...
      loadTranslations(settings.language)
        ...
        .catch(error => {
          logger.error('Failed to load translations for language:', settings.language, error);
  ...
  const preferences = useMemo<PreferencesState>(
    () => ({
      ...
      customAccentColor: settings.customAccentColor ?? null,
  ...
  const business = useMemo<BusinessState>(
    () => ({
      ...
      dashboardConfiguration: settings.dashboardConfiguration,
      customHairstyles: settings.customHairstyles || [],
    }),
    [settings]
  );

  const records = useMemo<RecordsState>(
    () => ({
      clients: profile.clients || [],
      appointments: profile.appointments || [],
      lookbooks: profile.lookbooks || [],
    }),
    [profile.clients, profile.appointments, profile.lookbooks]
  );

  const preferenceKeys: Array<keyof UserSettings> = useMemo(
    () => [
      'theme',
      'layout',
      'accentColor',
  ...
  const businessKeys: Array<keyof UserSettings> = useMemo(
    () => [
      'paymentTypes',
      'quickSaleItemIds',
      'coupons',
  ...
  const updatePreferences = useCallback(
    (values: Partial<PreferencesState>) => {
      const payload = preferenceKeys.reduce<Partial<UserSettings>>((acc, key) => {
        if (Object.prototype.hasOwnProperty.call(values, key)) {
          acc[key] = values[key as keyof PreferencesState] as any;
        }
        return acc;
      }, {});
    ...
    [preferenceKeys, updateSettings]
  );

  const updateBusinessSettings = useCallback(
    (values: Partial<BusinessState>) => {
      const payload = businessKeys.reduce<Partial<UserSettings>>((acc, key) => {
        if (Object.prototype.hasOwnProperty.call(values, key)) {
          acc[key] = values[key as keyof BusinessState] as any;
        }
        return acc;
      }, {});
  ...
  const contextValue: SettingsContextType = {
    ...
    stickyNotes: settings.stickyNotes || [],
    dashboardConfiguration:
      settings.dashboardConfiguration ||
      (DEFAULT_SETTINGS.dashboardConfiguration as DashboardConfiguration),
    ...
    setCustomAccentColor: (hexColor: string) =>
      updateSettings({ accentColor: 'custom', customAccentColor: hexColor }),
    ...
    setStudioInitialGenerations: (count: number) =>
      updateSettings({ studioInitialGenerations: count }),
    ...
    updateAppointmentDetails: async (appointmentId, updates) => {
      ...
      if (updates.durationOverride !== undefined)
        dbUpdates.duration_override = updates.durationOverride;
    ...
    updateServiceGroup: async group => {
      ...
      setProfile(p => ({
        ...
        settings: {
          ...p.settings,
          serviceGroups: (p.settings?.serviceGroups || []).map(g =>
    ...
    deleteServiceGroup: async groupId => {
      if (!user) throw new Error('User not authenticated');
      const { error } = await supabase.from('service_groups').delete().eq('id', groupId);
    ...
    updateHairstylist: hairstylist => {
      const newStylists = (settings.hairstylists || []).map(h =>
        h.id === hairstylist.id ? hairstylist : h
      );
    ...
    getHairstylistPerformance: (hairstylistId, month) => {
      ...
      return (
        hairstylist.performance?.find(p => p.month === currentMonth) ||
        hairstylist.performance?.[hairstylist.performance.length - 1] ||
        null
      );
    },
    ...
    updateProduct: async product => {
      ...
      setProfile(p => ({
        ...
        settings: {
          ...p.settings,
          products: (p.settings?.products || []).map(prod =>
    ...
    updateSegment: segment => {
      updateSettings({
        segments: (settings.segments || []).map(s => (s.id === segment.id ? segment : s)),
      });
    },
    ...
    connectSocialMedia: (platform, handle) =>
      updateSettings({ socialMedia: { ...settings.socialMedia, [platform]: handle } }),
    disconnectSocialMedia: platform =>
      updateSettings({ socialMedia: { ...settings.socialMedia, [platform]: null } }),
