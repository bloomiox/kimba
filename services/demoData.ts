...
export const generateDemoClients = (count: number): Client[] => {
  ...
  while (clients.length < count) {
    ...
    clients.push({
      ...
      notes:
        Math.random() > 0.7
          ? 'Prefers morning appointments and is allergic to lavender scents.'
          : 'Likes to chat during appointments. Favorite topics: movies and travel.',
...
export const generateDemoAppointments = (
  count: number,
  clients: Client[],
  services: Service[],
